...
\newcommand{\lectureTitle}{Authentisierung und Verschlüsselung [unvollständig]}
...
\begin{tikzpicture}
  ...
  \node (A_key) at (0,1) {$m_1$};

  \node[draw=black, line width=1pt] (B) at (2,0) {$F$};
  \node (B_key) at (2,1) {$m_2$};

  \node (C) at (4,0) {$\dots$};

  \node[draw=black, line width=1pt] (D) at (6,0) {$F$};
  \node (D_key) at (6,1) {$|m|$};
  \node (END) at (8,0) {$Z_\ell$};

  \draw[->] (START) -- (A);
  \draw[->] (A) -- node [midway,above] {$Z_1$} (B);
  \draw[->] (B) -- node [midway,above] {$Z_2$} (C);
  \draw[->] (C) -- node [midway,above] {$Z_{\ell-1}$} (D);
  \draw[->] (D) -- (END);
...
\textbf{Eigenschaften:}
\begin{itemize}
  \item{\textbf{Length-Extension-Angriff}: Wenn Hashwert $H(X_1 \parallel X_2)$ bekannt ist, kann $H(X_1 \parallel X_2 \parallel X_3)$ berechnet werden}
  \item{Viele bekannte Hashfunktionen basieren auf Merkle-Damgård, z.B. MD5, SHA-1 und SHA-2}
\end{itemize}

\subsubsection{Secretly Keyed Hashfunctions}
Es sei $H$ eine Hashfunktion, die die Merkle-Damgård-Konstruktion nutzt und $F$ die Kompressionsfunktion in $H$. Wenn wir nun einen Schlüssel $\kappa$ in $H$ unterbringen wollen, können wir $\kappa$ als Initialisierungsvektor $IV$ verwenden:\par
\begin{tikzpicture}
  \node (START) at (-2,0) {$\kappa$};
...
\begin{itemize}
  \item{$Gen(1^k)$: \begin{flalign*}
                \kappa \coloneqq (\kappa_1, \kappa_2) \stackrel{\$}{\leftarrow} {\{0, 1\}}^k \times {\{0, 1\}}^k
              \end{flalign*}
        }
  \item{$Sign(\kappa, m)$: \begin{flalign*}
...
\begin{flalign*}
   & \Pr[\mathcal{A} \text{ gewinnt IND-CPA-Experiment}]                                                \\
...
\begin{tikzpicture}
  \node (A) at (0,6) {$\mathcal{C}^{\text{SKE}}_\text{IND-CPA}$};
  \node (B) at (5,6) {$\mathcal{A}$};
  \node (Orakel) at (10,6) {$\mathcal{O}$};
  \node[label={[align=center]below:$b \stackrel{?}{=} b'$\\ $\land$ \\ $|m_0| \stackrel{?}{=} |m_1|$}] (C) at (0,0) {};

  \draw[dashed] (A) -- (C);
  \draw[dashed] (B) -- (5,0);
  \draw[dashed] (Orakel) -- (10,0);

  \node[label={[align=right]left:$\kappa \leftarrow Gen(1^k)$\\$b \stackrel{\$}{\leftarrow} \{0,1\}$}] at (0,5) {};
  \node[label={left:$c^* \leftarrow Enc(\kappa, m_b)$}]  at (0,3) {};

  \node[label={right:$c_i \leftarrow Enc(\kappa, m_i)$}]  at (10,3.5) {};

  \node at (7.5,1.5) {\small{(Anfragen immer erlaubt)}};

  \draw[->,shorten >=5pt, shorten <=5pt] (5,4.5) -- (10,4) node[midway, above, sloped] {$m_i$};
  \draw[->,shorten >=5pt, shorten <=5pt] (10,3) -- (5,2.5) node[midway, above, sloped] {$c_i$};

  \draw[->,shorten >=5pt, shorten <=5pt] (5,4) -- (0,3.5) node[midway, above, sloped] {$m_0, m_1$};

  \draw[->,shorten >=5pt, shorten <=5pt] (0,2.5) -- (5,2) node[midway, above, sloped] {$c^*$};
  \draw[->,shorten >=5pt, shorten <=5pt] (5,1.5) -- (0,1) node[midway, above, sloped] {$b'$};
\end{tikzpicture}
...
\subsubsection{Visualisierung: IND-CCA2-Sicherheitsexperiment}
Bei dem IND-CCA2-Sicherheitsexperiment wird das Orakel $\mathcal{O}$ ebenfalls vom Challenger implementiert und ist in der Visualisierung zur besseren Lesbarkeit getrennt. Der Angreifer $\mathcal{A}$ darf zu jedem Zeitpunkt Verschlüsselungs-Anfragen und Entschlüsselungs-Anfragen an das Orakel stellen.\par
\begin{tikzpicture}
  \node (A) at (0,6) {$\mathcal{C}^{\text{SKE}}_\text{IND-CCA2}$};
  \node (B) at (5,6) {$\mathcal{A}$};
  \node (Orakel) at (10,6) {$\mathcal{O}$};
  \node[label={[align=center]below:$b \stackrel{?}{=} b'$ $\land$  $|m_0| \stackrel{?}{=} |m_1|$ \notice{$\land$} \notice{$c^* \neq c_j \forall j$}}] (C) at (0,0) {};
  ...
  \node[label={right:$c_i \leftarrow Enc(\kappa, m_i)$}]  at (10,4.25) {};
  \node[label={right:\notice{$m_j \leftarrow Dec(\kappa, c_j)$}}]  at (10,1.75) {};

  \node at (7.5,0) {\small{(Anfragen immer erlaubt)}};

  \draw[->,shorten >=5pt, shorten <=5pt] (5,5) -- (10,4.5) node[midway, above, sloped] {$m_i$};
  \draw[->,shorten >=5pt, shorten <=5pt] (10,4) -- (5,3.5) node[midway, above, sloped] {$c_i$};

  \draw[->,draw=noticeColor,shorten >=5pt, shorten <=5pt] (5,2.5) -- (10,2) node[midway, above, sloped] {\notice{$c_j$}};
  \draw[->,draw=noticeColor,shorten >=5pt, shorten <=5pt] (10,1.5) -- (5,1) node[midway, above, sloped] {\notice{$m_j$}};
...
\hyperref[sec:MACs]{MACs} stellen die \textbf{Integrität} von Nachrichten sicher, während symmetrische Verschlüsselung die \textbf{Vertraulichkeit} von Nachrichten sicherstellt. \highlight{Authentifizierte Verschlüsselung} stellt beide Eigenschaften sicher.

\subsubsection{Visualisierung: INT-CTXT-Sicherheitsexperiment für SKE}
\begin{tikzpicture}
  \node (A) at (0,6) {$\mathcal{C}_\text{INT-CTXT}$};
  \node (B) at (5,6) {$\mathcal{A}$};
  \node[label={[align=center]below:$Dec(\kappa, c^*) \neq \bot$?\\ $\land$ \\ $c^* \notin \{c_1, \dots, c_q\}$?}] (C) at (0,0) {};
  \node (D) at (5,0) {};

  \draw[dashed] (A) -- (C);
  \draw[dashed] (B) -- (D);

  \node[label={left:$\kappa \leftarrow Gen(1^k)$}] at (0,5) {};
  \node[label={left:$c_i \leftarrow Enc(\kappa, m_i)$}]  at (0,3) {};
  ...
  \draw[->,shorten >=5pt, shorten <=5pt] (0,2.5) -- (5,2) node[midway, above, sloped] {$c_i$};
  \draw[->,shorten >=5pt, shorten <=5pt] (5,1.5) -- (0,1) node[midway, above, sloped] {$c^*$};
\end{tikzpicture}
...
\subsubsection{Visualisierung: IND-CPA-Sicherheitsexperiment}
Der Angreifer $\mathcal{A}$ kann mit $pk$ selbst jederzeit Nachrichten verschlüsseln.\par
\begin{tikzpicture}
  \node (A) at (0,6) {$\mathcal{C}^{\text{PKE}}_\text{IND-CPA}$};
  \node (B) at (5,6) {$\mathcal{A}$};
  \node[label={[align=center]below:$b \stackrel{?}{=} b'$ $\land$ $|m_0| \stackrel{?}{=} |m_1|$}] (C) at (0,0.5) {};

  \draw[dashed] (A) -- (C);
  \draw[dashed] (B) -- (5,0.5);

  \node[label={[align=right]left:$(pk, sk) \leftarrow Gen(1^k)$\\$b \stackrel{\$}{\leftarrow} \{0,1\}$}] at (0,5) {};
  \node[label={left:$c^* \leftarrow Enc(pk, m_b)$}]  at (0,3) {};
...
\subsubsection{Definition: IND-CCA2 für PKE}
Ein PKE-Schema $\text{PKE} = (Gen, Enc, Dec)$ ist \textit{IND-CCA2-sicher}, wenn für alle PPT $\mathcal{A}$ gilt, dass
\begin{flalign*}
   & \Pr[\mathcal{A} \text{ gewinnt IND-CCA2-Experiment}]                                                                             \\
   & = \Pr[\mathcal{A}^{\mathcal{C}^{\text{PKE}}_\text{IND-CCA2}}(1^k) = b': b = b' \land |m_0| = |m_1| \land c^* \neq c_j \forall j] \\
   & \leq \frac{1}{2} + negl(k)
\end{flalign*}
für eine im Sicherheitsparameter $k$ vernachlässigbare Funktion $negl$.

\subsubsection{Visualisierung: IND-CCA2-Sicherheitsexperiment}
Das Entschlüsselungs-Orakel $\mathcal{O}$ wird ebenfalls vom Challenger implementiert.\par
\begin{tikzpicture}
  \node (A) at (0,6) {$\mathcal{C}^{\text{PKE}}_\text{IND-CCA2}$};
  \node (B) at (5,6) {$\mathcal{A}$};
  \node (Orakel) at (10,6) {$\mathcal{O}$};
  \node[label={[align=center]below:$b \stackrel{?}{=} b'$ $\land$  $|m_0| \stackrel{?}{=} |m_1|$ \notice{$\land$} \notice{$c^* \neq c_j \forall j$}}] (C) at (0,0.5) {};

  \draw[dashed] (A) -- (C);
  \draw[dashed] (B) -- (5,0.5);
  \draw[dashed] (Orakel) -- (10,0.5);

  \node[label={[align=right]left:$(pk, sk) \leftarrow Gen(1^k)$\\$b \stackrel{\$}{\leftarrow} \{0,1\}$}] at (0,5) {};
  \node[label={left:$c^* \leftarrow Enc(pk, m_b)$}]  at (0,3) {};

  \node[label={right:\notice{$m_j \leftarrow Dec(sk, c_j)$}}]  at (10,2.75) {};

  \node at (7.5,1) {\small{(Anfragen immer erlaubt)}};

  \draw[->,draw=noticeColor,shorten >=5pt, shorten <=5pt] (5,3.5) -- (10,3) node[midway, above, sloped] {\notice{$c_j$}};
  \draw[->,draw=noticeColor,shorten >=5pt, shorten <=5pt] (10,2.5) -- (5,2) node[midway, above, sloped] {\notice{$m_j$}};
...
\subsection{Signcryption}
\subsubsection{Problem bei der Konstruktion eines IND-CCA2-sicheren PKE-Schemas}
Wir wollen auch für PKE ein IND-CCA2-sicheres Verfahren konstruieren. Bei SKE konnten wir dafür \hyperref[sec:symmetrischeVerschluesselungAuthentifizierteVerschluesselung]{Encrypt-then-Mac} verwenden.\par
...
\begin{flalign*}
   ...
   & \forall m \in \mathcal{M}_k              \\
   & \forall id \in \mathcal{ID}_k            \\
